...
#Define IGR production bootstrap function:
  igr.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
      ...
      if (taxon == "Ceratopogonidae A" | taxon == "Ceratopogonidae B" | taxon == "Macropelopia" | taxon == "Eukiefferiella minor" | taxon == "Eukiefferiella claripennis" | taxon == "Diamesa bohemani-zernyi" |
            taxon == "Diamesa bertrami" | taxon == "Micropsectra" | taxon == "Parochlus" | taxon == "Thienemaniella" | taxon == "Orthocladius frigidus" | taxon == "Orthocladius oblidens" | taxon == "Metriocnemus" |
          taxon == "Rheocricotopus effusus" | taxon == "Chaetocladius" |taxon == "Limnophyes" | taxon == "Thaumaleidae"){
      ...
      else if (taxon == "Limnophora riparia" | taxon == "Dicranota" | taxon == "Tipula"){
        for (s in 1:length(sizes$mm)){
          for (d in 1:length(t.int)){
      ...
      for (b in 1:boot.num){
        ...
        int.P <- matrix(0, length(t.int), length(sizes$mm))
        if (taxon == "Ceratopogonidae A" | taxon == "Ceratopegonidae B" | taxon == "Macropelopia" | taxon == "Eukiefferiella minor" | taxon == "Eukiefferiella claripennis" | taxon == "Diamesa bohemani-zernyi" |
            taxon == "Diamesa bertrami" | taxon == "Micropsectra" | taxon == "Parochlus" | taxon == "Thienemaniella" | taxon == "Orthocladius frigidus" | taxon == "Orthocladius oblidens" | taxon == "Metriocnemus" |
            taxon == "Rheocricotopus effusus" | taxon == "Chaetocladius" |taxon == "Limnophyes" | taxon == "Thaumaleidae"){
          for (s in 1:length(sizes$mm)){
          for (d in 1:length(t.int)){
