...
#Define IGR production bootstrap function:
  igr.prod <- function(DATA, site, first.date, last.date, habitat, taxon, TEMP, wrap=F, LM.a, LM.b, LM.p.ash, g.a, g.b, g.c, g.d, min.growth=0, temp.corr, boot.num){
    ...
    if (length(TEMP) == length(t.int) & length(temp.corr) == length(t.int)){
        ...
        for (s in 1:length(sizes$mm)){
          for (d in 1:length(t.int)){
            if ((g.a + (g.b*log(sizes$AFDM[s]))+ g.c * TEMP[d]) > 0){
              int.P[d,s] <- int.B[d,s]*t.int[d]*((g.a + g.b*log(sizes$AFDM[s]))+ (g.c * TEMP[d]))		#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[d,s] <- int.B[d,s]*t.int[d]*min.growth	#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
      ...
      else if (taxon == "Radix balthica"){
        for (s in 1:length(sizes$mm)){
          for (d in 1:length(t.int)){
            if (((exp(g.a + g.b*TEMP[d] + g.c*log(sizes$AFDM[s]) + g.d*TEMP[d]*log(sizes$AFDM[s]))) > 0)){
              int.P[d,s] <- int.B[d,s]*t.int[d]*(exp(g.a + g.b*TEMP[d] + g.c*log(sizes$AFDM[s]) + g.d*TEMP[d]*log(sizes$AFDM[s])))	#Calculate interval production (interval biomass * interval duration * growth rate for each size class on each date * temp correction factor for each date)
            }
            else {
              int.P[d,s] <- int.B[d,s]*t.int[d]*min.growth			#Calculate interval production (interval biomass * interval duration * minimum growth rate if growth equation gives a zero or negative value)
            }
      ...
      else if (taxon == "Tub. 1" | "S. vittatum" | "Ostracod" | "Oribatid 1" | "Leech" | "Nematode" | "Sperchon sp." | "Lumb 3" | "Springtail" | "Clinocera"){
        for (s in 1:length(sizes$mm)){
          for (d in 1:length(t.int)){
      ...
      else print(paste("Growth equation not assigned for this taxon: ", taxon))
   ...
   if (taxon == "Midge 1" | "Midge 2"){
        ...
        int.P[int.P < 0] <- 0				#Set negative interval production values to zero
        Pintboots[b,] <- apply(int.P, 1, sum)		#Sum all size classes to calculate annual production for each interval
        if (wrap==T){					#If the user wants to calculate a "wrap-around" interval for the last interval...
      ...
      Pboots <- apply(Pintboots, 1, sum)			#Calculate bootstrap vector of annual production for all intervals combined
