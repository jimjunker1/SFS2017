...
st14.fix = which(st14_bugs$DATE == "07/26/12")
st14_bugs[st14.fix, "DATE"] = "06/29/12"
...
saveRDS(st14_bugs, file = "st14_bugs.rda")
...
source("C:/Users/Jim/Documents/Projects/Talk/SFS 2017/SFS2017/R Secondary Production-GC Example/Scripts/st14igr.prod_function.R")
...
#tax = read.table(file = "./SFS code/st14_taxa_info_LISA_mod2.txt", header = T, sep = "\t", quote = "", strip.white = T)
...
set.seed(123)
st14.out = wrapper.site.yr(DATA = st14_bugs, site = "ST14", habitat = "COBBLE", TEMP.COB = st14_temp1, TEMP.DEP = st14_temp1, TEMP.TAL = st14_temp1, first.date = "07/01/11", last.date = "06/29/12",
...
saveRDS(st14.int, file = "st14.int.rda")
...
st14.spp.ann.prod$spp.rank = seq(1, length(st14.spp.ann.prod$Species.rank), length = length(st14.spp.ann.prod$Species.rank))
st14.spp.ann.prod$spp.rank = reorder(st14.spp.ann.prod$spp.rank, - st14.spp.ann.prod$Mean.Annual.Prod)
st14.spp.ann.prod[which(st14.spp.ann.prod$Mean.Annual.Prod == 0),] <- NA
st14.spp.ann.prod = st14.spp.ann.prod[! is.na(st14.spp.ann.prod$spp.rank),]
st14.spp.ann.prod = st14.spp.ann.prod %>% mutate(spp.rank = dense_rank(-Mean.Annual.Prod))

saveRDS(st14.spp.ann.prod, file = "st14.spp.ann.prod.rda")

ggplot(st14.spp.ann.prod %>% arrange(spp.rank), aes(x = Species.rank, y = log(Mean.Annual.Prod))) + geom_point(size = 2) + geom_path(aes(group = 1), size = 1.5) +
